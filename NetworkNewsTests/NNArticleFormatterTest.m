...
- (void)testHeaderFormatting {
    ...
    NSArray *headers = [NNArticleFormatter headerArrayWithDate:date
                                                          from:@"test@test.com"
    ...
    for (NNHeaderEntry *entry in headers)
    {
        if ([entry.name isEqualToString:@"Date"])
        {
            ...
            XCTAssert([entry.value isEqualToString:[dateFormatter stringFromDate:date]], "Expecting 'Date' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"From"])
        {
            XCTAssert([entry.value isEqualToString:@"test@test.com"], "Expecting 'From' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"Newsgroups"])
        {
            XCTAssert([entry.value isEqualToString:@"misc.test"], "Expecting 'Newsgroups' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"Subject"])
        {
            XCTAssert([entry.value isEqualToString:@"Unit testing"], "Expecting 'Subject' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"References"])
        {
            XCTAssert([entry.value isEqualToString:@"<msg-ref-456> <msg-ref-789>"], "Expecting 'References' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"Reply-To"])
        {
            XCTAssert([entry.value isEqualToString:@"reply@test.com"], "Expecting 'Reply-To' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"Organization"])
        {
            XCTAssert([entry.value isEqualToString:@"Testing Inc"], "Expecting 'Organization' value");
            ++count;
        }
        else if ([entry.name isEqualToString:@"User-Agent"])
        {
            XCTAssert([entry.value hasPrefix:@"NetworkNews"], "Expecting 'User-Agent' value");
            ++count;
        }
    }
