...
- (void)testPostArticle {
  ...
  // Generate differing text so we don't get caught out with "duplicate message"
  // errors
  NSString *text =
      [NSString stringWithFormat:@"Testing at %@", [[NSDate date] description]];
...
- (void)testPostUTF8Article {
  ...
  NSString *text = [NSString
      stringWithFormat:@"📱 Testing at %@", [[NSDate date] description]];
  NSData *articleData = [NNArticleFormatter articleDataWithHeaders:headers
                                                              text:text
                                                      formatFlowed:YES];
