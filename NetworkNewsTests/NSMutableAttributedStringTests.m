...
- (void)testAppendNewsHead {
  NSArray *headers = @[
    [[NNHeaderEntry alloc] initWithName:@"Header-1" value:@"Value-1"],
    [[NNHeaderEntry alloc] initWithName:@"Header-2" value:@"Value-2"]
  ];
  NSMutableAttributedString *attrString =
      [[NSMutableAttributedString alloc] init];
  [attrString appendNewsHead:headers];
  XCTAssert([attrString.string
      isEqualToString:@"Header-1: Value-1\nHeader-2: Value-2\n"]);
}
...
- (void)testAppendShortNewsHead {
  ...
  XCTAssert([attrString.string
      isEqualToString:@"From: Value-From\nSubject: Value-Subject\n"]);
}
...
- (void)testAppendNewsBodyFlowed {
  ...
  XCTAssert([attrString.string
      isEqualToString:@"Fragment 1 Fragment 2\nFragment 3 Fragment 4\n"]);
}
