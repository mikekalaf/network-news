...
- (void)testLF {
    ...
    const char *withLF = "Lorem \nipsum \ndolor sit \namet, \nconsectetur \nadipisicing \nelit";
    XCTAssertEqual(textData.length, strlen(withLF), "ASCII string lengths are different");
    XCTAssertEqual(strncmp(textData.bytes, withLF, 62), 0, "ASCII strings don't match");

    NSData *textDataWithCRLF = [textData dataWithCRLFs];
    const char *withCRLF = "Lorem \r\nipsum \r\ndolor sit \r\namet, \r\nconsectetur \r\nadipisicing \r\nelit";
    XCTAssertEqual(textDataWithCRLF.length, strlen(withCRLF), "ASCII string lengths are different");
    XCTAssertEqual(strncmp(textDataWithCRLF.bytes, withCRLF, 68), 0, "ASCII strings don't match");
}

- (void)testCR {
    // Any stray CRs should have LFs appended (should be less likely)
    NSString *text = @"Lorem ipsum \rdolor sit amet, \rconsectetur adipisicing elit";
    NSData *textData = [text dataUsingEncoding:NSASCIIStringEncoding];
    NSData *textDataWithCRLF = [textData dataWithCRLFs];
    const char *withCRLF = "Lorem ipsum \r\ndolor sit amet, \r\nconsectetur adipisicing elit";
    XCTAssertEqual(textDataWithCRLF.length, strlen(withCRLF), "ASCII string lengths are different");
    XCTAssertEqual(strncmp(textDataWithCRLF.bytes, withCRLF, 60), 0, "ASCII strings don't match");
}
