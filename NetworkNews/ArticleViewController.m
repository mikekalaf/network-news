...
@interface ArticleViewController () <NewArticleDelegate,
...
- (void)viewDidLoad {
  ...
  //    constraintsWithVisualFormat:@"|[_textView]|"
  //                                                                   options:0
  //                                                                   metrics:nil
  //                                                                     views:views];
  ...
  UIBarButtonItem *nextButton = [[UIBarButtonItem alloc]
      initWithImage:[UIImage imageNamed:@"button-down-arrow"]
              style:UIBarButtonItemStylePlain
             target:self
             action:@selector(nextArticlePressed:)];
  UIBarButtonItem *previousButton = [[UIBarButtonItem alloc]
      initWithImage:[UIImage imageNamed:@"button-up-arrow"]
              style:UIBarButtonItemStylePlain
             target:self
...
- (void)viewWillDisappear:(BOOL)animated {
  ...
  if ([currentViewController isKindOfClass:[ThreadViewController class]]) {
    ...
    ThreadViewController *threadViewController =
        (ThreadViewController *)currentViewController;
    ...
    [threadViewController returningFromArticleIndex:_articleIndex];
                 ...
                 isKindOfClass:[ThreadListViewController class]]) {
    // So the current article is within view on returning
    ThreadListViewController *threadListViewController =
        (ThreadListViewController *)currentViewController;
...
- (void)viewWillLayoutSubviews {
  [_textView
      setContentOffset:CGPointMake(0, -_textView.adjustedContentInset.top)
              animated:NO];
  if (_textAttachment) {
...
- (void)setGroupName:(NSString *)groupName {
  ...
  if (_groupName) {
    //_cacheURL = [[[_connectionPool account] cacheURL]
    //URLByAppendingPathComponent:_groupName];

    // Let's put all articles into the same directory, which will make it easier
    // to clean up plus any cross-postings will share the same cache
    _cacheURL = [_connectionPool.account.cacheURL
...
- (IBAction)replyButtonPressed:(id)sender {
  ...
  if (followupTo &&
      [followupTo caseInsensitiveCompare:@"poster"] == NSOrderedSame) {
    [self replyViaEmail];
    return;
  }
...
//    NSUInteger completePartCount = [[_article completePartCount]
//    integerValue]; if (completePartCount == 1 && _attachmentURL == nil)
//    {
//        // NOP
//    }
...
//    // can't assume that the article has in fact been read. The above code,
//    and
//    // all other code that updates and displays article HTML should be
//    factored-out
...
//    sharedApplication] delegate]; [appDelegate.activeCoreDataStack save];
//}
//
...
////
////    [htmlString appendString:@"<p class=\"status\">Article
///Unavailable</p>"];
////
////    [self endHTML];
////    [_webView loadHTMLString:htmlString baseURL:nil];
...
- (void)fetchArticleCompleted:(NSNotification *)notification {
  ...
  if (statusCode == 220 || statusCode == 222) {
    if ([userInfo[@"partNumber"] integerValue] ==
        [userInfo[@"totalPartCount"] integerValue]) {
...
- (NSString *)bodyTextForFollowUp {
  ...
  // - Drop any signature
  NNQuoteLevelParser *qlp =
      [[NNQuoteLevelParser alloc] initWithData:_bodyTextDataTop flowed:YES];
  NSArray *quoteLevels = qlp.quoteLevels;

  NSMutableString *mutableString =
      [NSMutableString stringWithCapacity:_bodyTextDataTop.length];
...
- (void)updateContent {
  ...
  if (_attachmentURL) {
    ...
    NSData *attachmentData =
        [[NSData alloc] initWithContentsOfURL:_attachmentURL];
    ...
    [attrString appendAttributedString:
                    [NSAttributedString
                        attributedStringWithAttachment:_textAttachment]];
  } else {
    _textAttachment = nil;
  }
...
- (NSArray *)cachedURLsForMessageID:(NSString *)messageID {
  ...
  NSError *error;
  NSArray *contents = [fileManager
        contentsOfDirectoryAtURL:_cacheURL
  ...
  NSPredicate *predicate =
      [NSPredicate predicateWithFormat:@"lastPathComponent CONTAINS %@",
...
- (void)loadArticle {
  ...
  //    NSString *sizeString = [formatter stringForObjectValue:[_article
  //    totalByteCount]];

  //    if ([_article hasAllParts] == NO)
  //    {
  //        [htmlString appendString:@"<p class=\"status\">Incomplete
  //        parts</p>"];
  //    }
  //    else
  //        [htmlString appendFormat:@"<p class=\"status\">Loading (%@)...</p>",
  //        sizeString];
  ...
  NSSortDescriptor *descriptor =
      [[NSSortDescriptor alloc] initWithKey:@"partNumber" ascending:YES];
  NSArray *sortedParts =
      [_article.parts.allObjects sortedArrayUsingDescriptors:@[ descriptor ]];
  ...
  if (URLs.count) {
    ...
    for (ArticlePart *part in sortedParts) {
      FetchArticleOperation *operation = [[FetchArticleOperation alloc]
          initWithConnectionPool:_connectionPool
                        ...
                        progress:^(NSUInteger bytesReceived) {
                          ...
                          dispatch_async(dispatch_get_main_queue(), ^{
                            self->_progressView.progress =
                                (float)bytesFetchedSoFar / totalBytes;
                          });
                        }];
...
- (void)updateTitle {
  self.title = [NSString
      stringWithFormat:@"%ld of %lu", (unsigned long)_articleIndex + 1,
                       (unsigned long)[_articleSource articleCount]];
}

- (void)updateNavigationControls {
  // Enable/disable the navigation controls
  (self.navigationItem.rightBarButtonItems[0]).enabled =
      (_articleIndex < [_articleSource articleCount] - 1);
...
- (void)showProgressToolbar {
  ...
  UIBarButtonItem *progressItem =
      [[UIBarButtonItem alloc] initWithCustomView:_progressView];

  self.toolbarItems =
      [NSArray arrayWithObjects:flexibleSpaceButtonItem, progressItem,
...
- (void)showArticleToolbar {
  ...
  UIBarButtonItem *aReplyButtonItem = [[UIBarButtonItem alloc]
      initWithBarButtonSystemItem:UIBarButtonSystemItemReply
                           target:self
                           action:@selector(replyButtonPressed:)];

  self.toolbarItems =
      [NSArray arrayWithObjects:flexibleSpaceButtonItem, aReplyButtonItem, nil];
}
