...
+ (instancetype)accountWithTemplate:(AccountTemplate)accountTemplate {
  ...
  case AccountTemplateEternalSeptember:
    ...
    account.supportURL =
        [NSURL URLWithString:@"https://www.eternal-september.org"];
    account.iconName = @"es";
...
- (instancetype)initWithCoder:(NSCoder *)decoder {
  ...
  if (self) {
    _accountTemplate =
        (AccountTemplate)[decoder decodeIntegerForKey:@"accountTemplate"];
...
- (NSURL *)cacheURL {
  if (_cacheURL == nil) {
    ...
    NSURL *rootCacheURL = [fileManager URLsForDirectory:NSCachesDirectory
