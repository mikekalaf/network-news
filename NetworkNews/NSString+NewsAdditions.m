  ...
  while (range.location < self.length) {
    ...
    if ([self characterAtIndex:range.location] == '>') {
      ...
      if (lfRange.location != NSNotFound) {
        NSRange lineRange =
            NSMakeRange(range.location, lfRange.location - range.location);
    ...
    if (self.length - range.location > location) {
      ...
      if (spaceRange.location != NSNotFound) {
        NSRange lineRange = NSMakeRange(range.location, spaceRange.location -
                                                            range.location + 1);
        [ranges addObject:[NSValue valueWithRange:lineRange]];
        range.location = spaceRange.location + 1;
        softLineBreak = YES;

        //                NSLog(@"soft line: '%@'", [self
        //                substringWithRange:lineRange]);
        continue;
      }
    }
    ...
    if (range.location + range.length < self.length) {
      ...
      NSRange extendedRange =
          NSMakeRange(range.location, self.length - range.location);
      NSRange overRange =
          [self rangeOfCharacterFromSet:[NSCharacterSet
      ...
      if (overRange.location != NSNotFound) {
        ...
        NSRange lineRange = NSMakeRange(
            range.location, overRange.location - range.location + adjust);
        [ranges addObject:[NSValue valueWithRange:lineRange]];
...
- (NSString *)messageIDFileName {
  ...
  NSMutableCharacterSet *pathCharSet =
...
- (NSString *)stringByReplacingOccurrencesOfNumbersWithString:
  ...
  while (searchRange.length) {
    NSRange range =
        [self rangeOfCharacterFromSet:[NSCharacterSet decimalDigitCharacterSet]
                              options:0
                                range:searchRange];
