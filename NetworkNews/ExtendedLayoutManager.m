...
- (void)drawGlyphsForGlyphRange:(NSRange)glyphsToShow atPoint:(CGPoint)origin {
  ...
  while (glyphRange.length > 0) {
    ...
    NSRange attributeGlyphRange =
        [self glyphRangeForCharacterRange:attributeCharRange
    ...
    if (attribute) {
      ...
      NSTextContainer *container =
          [self textContainerForGlyphAtIndex:attributeGlyphRange.location
                              effectiveRange:NULL];
      ...
      if (indent > 0) {
        ...
        for (NSUInteger i = 0; i < level; ++i) {
          ...
          CGRect rect = {{LEVEL_INDENT * i + 5, paraRect.origin.y + 8},
      ...
      if (spacing > 0) {
        ...
        CGRect rect = {{5, paraRect.origin.y - 5}, {paraRect.size.width, 1}};
        CGContextFillRect(context, rect);
      }
    }
    ...
    glyphRange.length =
        NSMaxRange(glyphRange) - NSMaxRange(attributeGlyphRange);
