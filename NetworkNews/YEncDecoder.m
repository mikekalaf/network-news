...
static NSUInteger scanForPair(const char *bytes, NSUInteger startOffset,
                              NSUInteger length) {
  for (NSUInteger i = startOffset; i < length - 4; ++i) {
    if (i == startOffset && bytes[i] == '=' && bytes[i + 1] == 'y')
      return i;

    if (bytes[i] == 13 && bytes[i + 1] == 10 && bytes[i + 2] == '=' &&
        bytes[i + 3] == 'y')
      return i + 2;
  }
...
static NSUInteger scanForNextLine(const char *bytes, NSUInteger startOffset,
                                  NSUInteger length) {
...
static void scanForBeginFields(const char *bytes, NSUInteger startOffset,
                               NSUInteger length, NSString **fileName,
                               NSUInteger *part) {
  NSUInteger fieldNameOffset = startOffset;
  for (NSUInteger i = startOffset; i < length; ++i) {
    if (bytes[i] == '=') {
      if ((i - fieldNameOffset == 4) && bytes[fieldNameOffset] == 'n' &&
        ...
        *fileName = [[NSString alloc] initWithBytes:bytes + i + 1
        ...
        return;
      } else if ((i - fieldNameOffset == 4) && bytes[fieldNameOffset] == 'p' &&
                 bytes[fieldNameOffset + 1] == 'a' &&
...
static void scanForEndFields(const char *bytes, NSUInteger startOffset,
                             NSUInteger length, NSUInteger *size,
                             NSUInteger *crc32) {
  NSUInteger fieldNameOffset = startOffset;
  for (NSUInteger i = startOffset; i < length; ++i) {
    if (bytes[i] == '=') {
      if ((i - fieldNameOffset == 4) && bytes[fieldNameOffset] == 's' &&
          bytes[fieldNameOffset + 1] == 'i' &&
          bytes[fieldNameOffset + 2] == 'z' &&
          bytes[fieldNameOffset + 3] == 'e') {
        *size = strtoll(bytes + i + 1, NULL, 10);
      } else if ((i - fieldNameOffset == 5) && bytes[fieldNameOffset] == 'c' &&
...
static NSUInteger beginOfData(const char *bytes, NSUInteger startOffset,
                              NSUInteger length, NSString **fileName,
                              NSUInteger *part,
  ...
  while (offset < length) {
    ...
    if (offset < length - 7) {
      if (bytes[offset + 2] == 'b' && bytes[offset + 3] == 'e' &&
          bytes[offset + 4] == 'g' && bytes[offset + 5] == 'i' &&
          bytes[offset + 6] == 'n' && bytes[offset + 7] == ' ') {
        NSUInteger next = scanForNextLine(bytes, offset + 8, length);
        NSString *thisFileName;
        NSUInteger thisPart = 0;
        scanForBeginFields(bytes, offset + 8, next - 2, &thisFileName,
                           &thisPart);
...
static NSUInteger endOfData(const char *bytes, NSUInteger startOffset,
                            NSUInteger length, NSUInteger *size,
                            NSUInteger *crc32, NSUInteger *endIncludingTail) {
  NSUInteger offset = startOffset;
  while (offset < length) {
    offset = scanForPair(bytes, offset, length);
    if (offset < length - 5) {
      if (bytes[offset + 2] == 'e' && bytes[offset + 3] == 'n' &&
          bytes[offset + 4] == 'd' && bytes[offset + 5] == ' ') {
...
- (NSData *)decode {
  ...
  if (begin < NSUIntegerMax) {
    ...
    if (end < NSUIntegerMax) {
      ...
      // Determine the range of the encoded data, including the header and
      // footer
