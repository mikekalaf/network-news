...
{
    NSLog(@"ThreadListViewController restoreLevelWithSelectionArray: %@", aSelectionArray);

    // Load cached threads
    AppDelegate *appDelegate = (AppDelegate *)[[UIApplication sharedApplication] delegate];
    NSString *path = [appDelegate.cacheRootDir stringByAppendingPathComponent:@"threads.archive"];
    threads = [NSKeyedUnarchiver unarchiveObjectWithFile:path];
    
    // Are we to restore a search?
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    searchText = [userDefaults objectForKey:MOST_RECENT_ARTICLE_SEARCH];
    searchScope = [userDefaults integerForKey:MOST_RECENT_ARTICLE_SEARCH_SCOPE];
    
    if (threads)
    {
        // If the thread count is zero, then something had gone wrong with the
        // cached threads, so abandon the restore at this point
        if (threads.count == 0)
        {
            threads = nil;
            return;
        ...
        // Break things off here if we have a search
        // TODO: Restore to the article
        if (searchText)
            return;
        ...
        if (index == -2)
        {
            // Because the new article view is a modal view, we must defer
            // restoring it until this view has loaded
