...
static NSUInteger endOfLine(const char *bytes, NSUInteger startOffset,
                            NSUInteger length) {
...
static NSUInteger scanForUuencodedData(const char *bytes,
  ...
  for (NSUInteger i = startOffset; i < length - 11; ++i) {
    if (bytes[i] == 'b' && bytes[i + 1] == 'e' && bytes[i + 2] == 'g' &&
        bytes[i + 3] == 'i' && bytes[i + 4] == 'n' && bytes[i + 5] == ' ' &&
        isdigit(bytes[i + 6]) && isdigit(bytes[i + 7]) &&
        isdigit(bytes[i + 8]) && bytes[i + 9] == ' ') {
      ...
      return next;
    } else if (bytes[i] == 13 && bytes[i + 1] == 10 && bytes[i + 2] == 'b' &&
               bytes[i + 3] == 'e' && bytes[i + 4] == 'g' &&
               bytes[i + 5] == 'i' && bytes[i + 6] == 'n' &&
               bytes[i + 7] == ' ' && isdigit(bytes[i + 8]) &&
               isdigit(bytes[i + 9]) && isdigit(bytes[i + 10]) &&
               bytes[i + 11] == ' ') {
...
static NSUInteger stopOfData(const char *bytes, NSUInteger startOffset,
                             NSUInteger length, NSUInteger *endIncludingTail) {
  for (NSUInteger i = startOffset; i < length - 6; ++i) {
    if (bytes[i] == 13 && bytes[i + 1] == 10 && bytes[i + 2] == 'e' &&
        bytes[i + 3] == 'n' && bytes[i + 4] == 'd' && bytes[i + 5] == 13 &&
        bytes[i + 6] == 10) {
      if (endIncludingTail)
  ...
  for (NSUInteger i = startOffset; i < length - 4; ++i) {
    if (bytes[i] == 10 && bytes[i + 1] == 'e' && bytes[i + 2] == 'n' &&
        bytes[i + 3] == 'd' && bytes[i + 4] == 10) {
      if (endIncludingTail)
...
- (NSData *)decode {
  ...
  if (begin < NSUIntegerMax) {
    ...
    if (end < NSUIntegerMax) {
      NSLog(@"begin: %lu (%lu), end: %lu (%lu)", (unsigned long)begin,
            (unsigned long)beginOfBlock, (unsigned long)end,
        ...
        if (bytes[i] == 10 || bytes[i] == 13) {
          if (byteCount > 0) {
            ...
            if (byteCount <= bufIndex && bufIndex < byteCount + 3) {
              ...
              NSLog(@"Mismatched data in line whilst uudecoding (byteCount: "
                    @"%d, bufIndex: %d)",
                    byteCount, bufIndex);
              break;
            }
          ...
          if (bufIndex >= 45) {
            ...
            //                        stringWithCString:testBuffer length:60];
            //                        NSLog(@"Line: %@", str);
            ...
            buffer[bufIndex] =
                ((bytes[i] - 32) << 2) | (((bytes[i + 1] - 32) & 63) >> 4);
            buffer[bufIndex + 1] =
                ((bytes[i + 1] - 32) << 4) | (((bytes[i + 2] - 32) & 63) >> 2);
            buffer[bufIndex + 2] =
                ((bytes[i + 2] - 32) << 6) | ((bytes[i + 3] - 32) & 63);
