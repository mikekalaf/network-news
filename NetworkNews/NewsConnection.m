...
NSString *const NewsConnectionBytesReceivedNotification =
    @"NewsConnectionBytesReceivedNotification";
...
- (instancetype)initWithHost:(NSString *)host
  ...
  if (self) {
    ...
    if (response.statusCode == 200) {
      _welcome = [[NSString alloc] initWithData:response.data
...
- (NewsResponse *)readResponseWithCode:(NSInteger)terminatingStatusCode {
  ...
  while (YES) {
    ...
    if (bytesRead > 0) {
      ...
      if (statusCode == 0 && isdigit(buffer[0]) && isdigit(buffer[1]) &&
          isdigit(buffer[2]))
        statusCode = 100 * (buffer[0] - '0') + 10 * (buffer[1] - '0') +
                     (buffer[2] - '0');

      //            NSString *str = [[NSString alloc] initWithBytes:buffer
      //            length:bytesRead encoding:NSUTF8StringEncoding];
...
- (BOOL)isMultilineTerminatedData:(NSData *)data {
  ...
  if (len >= 5 && bytes[len - 5] == 13 && bytes[len - 4] == 10 &&
      bytes[len - 3] == '.' && bytes[len - 2] == 13 && bytes[len - 1] == 10) {
    return YES;
  }
...
- (NewsResponse *)capabilities {
  ...
  if (response.statusCode == 101) {
    NSArray *capabilities =
        [[response string] componentsSeparatedByString:@"\r\n"];
