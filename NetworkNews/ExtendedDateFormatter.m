...
- (NSString *)stringFromDate:(NSDate *)date {
  ...
  if (timeInterval <= dateTimeInterval &&
      dateTimeInterval < (timeInterval + 86400)) {
    // Today
    self.dateStyle = NSDateFormatterNoStyle;
    self.timeStyle = NSDateFormatterShortStyle;
  } else if ((timeInterval - 86400) <= dateTimeInterval &&
             dateTimeInterval < timeInterval) {
    // Yesterday
    return @"Yesterday";
  } else if ((timeInterval + 86400) <= dateTimeInterval &&
             dateTimeInterval < (timeInterval + 2 * 86400)) {
    // Tomorrow
    return @"Tomorrow";
  } else if ((timeInterval - 6 * 86400) <= dateTimeInterval &&
             dateTimeInterval < (timeInterval - 86400)) {
