...
NSString *FetchArticleCompletedNotification =
    @"FetchArticleCompletedNotification";
  ...
  @try {
    ...
    do {
      ...
      if (response.statusCode == 220 || response.statusCode == 222) {
        ...
        NSLog(@"%@", [[NSString alloc] initWithData:response.data
...
- (NSURL *)cacheURLForMessageID:(NSString *)messageID
  ...
  NSString *fileName =
      [NSString stringWithFormat:@"%@.%03lu", [messageID messageIDFileName],
                                 (unsigned long)order];
  return [_cacheURL URLByAppendingPathComponent:
...
- (void)processBody:(NSData *)data {
  ...
  NSString *contentTransferEncoding =
      [self headerValueWithName:@"Content-Transfer-Encoding"];

  Attachment *attachment =
      [[Attachment alloc] initWithBodyData:bodyData
  ...
  if (attachment) {
    if (_partNumber == 1) {
      ...
      NSURL *attachmentURL =
          [self cacheURLForMessageID:_messageID
                               order:2
    ...
    if (_partNumber == 1) {
      // Create the file
      NSError *error;
      if ([attachment.data writeToURL:attachmentURL options:0
                                error:&error] == NO) {
      ...
      NSFileHandle *fileHandle =
          [NSFileHandle fileHandleForWritingToURL:attachmentURL error:NULL];
    ...
    if (_partNumber == 1) {
      BOOL quotedPrintable =
          [QuotedPrintableDecoder isQuotedPrintable:_headEntries];

      if (quotedPrintable) {
        QuotedPrintableDecoder *quotedPrintableDecoder =
            [[QuotedPrintableDecoder alloc] init];
