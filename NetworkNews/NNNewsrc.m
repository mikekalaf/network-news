...
- (void)setRead:(BOOL)read
  ...
  if (precedingRange == nil && followingRange == nil) {
    // New range
    [item.articleRanges
        addObject:[NSValue valueWithArticleRange:ArticleRangeMake(number, 0)]];
...
- (void)setSubscribedGroupNames:(NSArray *)groupNames {
  ...
  NSMutableArray *rebuiltGroups =
      [NSMutableArray arrayWithCapacity:groupNames.count];
...
- (void)writeNewsrcFileAtURL:(NSURL *)url {
  ...
  [stream open];
  for (NNNewsrcItem *item in _groups) {
    [stream write:(const uint8_t *)[item.groupName
    ...
    for (NSValue *value in item.articleRanges) {
      ...
      if (range.length == 0)
        ...
        snprintf(buf, 255, "%llu-%llu", range.location,
